...
test.describe("Jasmine panels and tools", () => {
  ...
  test("right panel artifacts and context taxonomy update from the current thread", async () => {
    ...
    await expect(page.locator(".taxonomy-summary")).toContainText("schema v4");
    await expect(page.locator(".taxonomy-summary")).toContainText("payload sha256");
    await expect(page.locator(".taxonomy-warning-card")).toHaveCount(0);
    await expect(page.locator(".taxonomy-summary-counts")).toContainText("1 tools");
    await expect(page.locator(".taxonomy-summary-counts")).toContainText("1 options");
    await expect(page.locator(".taxonomy-summary-counts")).toContainText("pieces");
    await expect(page.locator(".taxonomy-composition")).toContainText("Composition");
    await expect(page.locator(".taxonomy-cache-card")).toContainText("Cache evidence");
    ...
    await expect(rawPayload).toContainText("Raw provider payload");
    ...
    await expect(systemTaxonomy).toContainText("presentation item 1");
    await expect(systemTaxonomy).toContainText("$.messages[0]");
    await expect(systemTaxonomy).toContainText("System prompt");
    await expect(systemTaxonomy.locator(".taxonomy-segment", { hasText: "System prompt" })).toBeVisible();
    const projectContext = systemTaxonomy.locator(".taxonomy-segment", { hasText: "Project context" });
    await expect(projectContext).toBeVisible();
    await projectContext.locator("summary").click();
    await expect(projectContext.locator("pre")).toContainText("Jasmine Agent Instructions");
    const skillInstructions = systemTaxonomy.locator(".taxonomy-segment", { hasText: "Skill instructions" });
    await skillInstructions.locator("summary").click();
    await expect(skillInstructions.locator("pre")).toContainText("ui-ux-product-harness");
    ...
    await expect(toolTaxonomy).toContainText("provider.payload.tools");
    await expect(toolTaxonomy.locator(".taxonomy-item-details").first()).toHaveAttribute("open", "");
    await expect(toolTaxonomy.locator(".taxonomy-segment", { hasText: "Tool definition" })).toBeVisible();
    ...
    await expect(optionsTaxonomy).toContainText("provider.payload.options");
