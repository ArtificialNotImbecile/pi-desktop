...
| [x] | WF-002 | Continue and manage a conversation | Drafts, paging, rename, search, edit, regenerate, queue, stop, and deletion affect the intended thread and its Pi session only; retry/edit retain inactive JSONL branches; model-switched history is passed to Pi without app-specific request filtering; context usage follows Pi's full-session usage and compaction rules rather than loaded UI text; canonical JSONL, collapsible history, content-only DeepSeek tool turns, and per-turn reasoning metadata restore correctly after restart. | `tests/unit/pi-runtime-equivalence.mjs`, `tests/unit/pi-context-usage.mjs`, `tests/e2e/threads.spec.ts`, `tests/e2e/composer.spec.ts`, `tests/e2e/chat-runtime.spec.ts`, `tests/e2e/rendering.spec.ts` |
