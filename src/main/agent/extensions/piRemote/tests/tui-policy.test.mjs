...
test("daemon reuse requires both the current runtime version and exact artifact", () => {
  ...
  assert.equal(daemonMatchesRuntime({ runtimeVersion: "0.1.2", artifactSha256 }, { artifactSha256 }), true);
  assert.equal(daemonMatchesRuntime({ runtimeVersion: "0.0.9", artifactSha256 }, { artifactSha256 }), false);
  assert.equal(daemonMatchesRuntime({ runtimeVersion: "0.1.2", artifactSha256: "b".repeat(64) }, { artifactSha256 }), false);
  assert.equal(daemonMatchesRuntime({ runtimeVersion: "0.1.2" }, { artifactSha256 }), false);
});
